...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `HALLO @${num.split('@')[0]}\hallo beban grup selamat datang di *${mdata.subject}* semoga betah disini jngn lupa baca rules🖤`
        ...
        .catch(
			...
			case 'loli': 
				...
				case 'url2img': 
					...
					if (!tipelist.includes(args[0])) return reply('𝗧𝗶𝗽𝗲 𝗱𝗲𝘀𝗸𝘁𝗼𝗽|𝘁𝗮𝗯𝗹𝗲𝘁|𝗺𝗼??𝗶𝗹𝗲')
...
break
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮🗣️KAUM HALU BROADCAST🗣️❯\n\n${body.slice(4)}`})
						}
						reply('Berhasil Broadcast ')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `❮📢️KAUM HALU BROADCASTBROADCAST 📢️❯\n\n${body.slice(4)}`)
						}
						reply('Berhasil Broadcast ')
					}
					break
       ...
       client.sendMessage(from, 'Itu kak nomor owner ku ⚜️Bryan Bot⚜️,Di save ya kak pasti di save back kok  ><',MessageType.text, { quoted: mek} )
           break    
           case 'demote':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
